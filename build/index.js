...
const isSearchArgs = (args) => typeof args === 'object' && args !== null && typeof args.q === 'string';
const isListFamiliesArgs = (args) => typeof args === 'object' &&
    args !== null &&
    (typeof args.brand_id === 'string' || typeof args.brand_id === 'number');
const isListWatchesArgs = (args) => typeof args === 'object' &&
    args !== null &&
    (typeof args.brand_id === 'string' || typeof args.brand_id === 'number') &&
...
const isGetWatchDetailsArgs = (args) => typeof args === 'object' &&
    args !== null &&
...
class WatchBaseServer {
    ...
    setupToolHandlers() {
        ...
        const tools = [
            {
                name: 'search',
                description: 'Search the database by brand name, family name, watch name and reference number (whole words).',
                inputSchema: {
                    type: 'object',
                    properties: {
            ...
            {
                name: 'search_refnr',
                description: 'Search the database by reference number (allows partial matches).',
                inputSchema: {
                    type: 'object',
                    properties: {
            ...
            {
                name: 'list_watches',
                description: 'Retrieve a list of watches for a particular Brand and/or Family, optionally filtered by update date.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        brand_id: {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                switch (name) {
                    ...
                    case 'search_refnr':
                        if (!isSearchArgs(args))
                            throw new McpError(ErrorCode.InvalidParams, 'Invalid arguments for search_refnr');
                    ...
                    case 'list_brands':
                        ...
                        break;
                    case 'list_families':
                        if (!isListFamiliesArgs(args))
                            throw new McpError(ErrorCode.InvalidParams, 'Invalid arguments for list_families');
                        apiPath = 'families';
                        apiParams = { 'brand-id': args.brand_id }; // API uses hyphen
                        break;
                    case 'list_watches':
                        if (!isListWatchesArgs(args))
                            throw new McpError(ErrorCode.InvalidParams, 'Invalid arguments for list_watches');
                        apiPath = 'watches';
                        apiParams = { 'brand-id': args.brand_id }; // API uses hyphen
                    ...
                    case 'get_watch_details':
                        if (!isGetWatchDetailsArgs(args))
                            throw new McpError(ErrorCode.InvalidParams, 'Invalid arguments for get_watch_details');
                        apiPath = 'watch';
